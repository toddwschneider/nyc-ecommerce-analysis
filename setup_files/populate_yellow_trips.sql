...
FROM
  yellow_tripdata_staging
    ...
    LEFT JOIN tmp_pickups ON yellow_tripdata_staging.id = tmp_pickups.id
      LEFT JOIN nyct2010_taxi_zone_mapping map_pickups ON tmp_pickups.gid = map_pickups.nyct2010_gid
    LEFT JOIN tmp_dropoffs ON yellow_tripdata_staging.id = tmp_dropoffs.id
      LEFT JOIN nyct2010_taxi_zone_mapping map_dropoffs ON tmp_dropoffs.gid = map_dropoffs.nyct2010_gid;

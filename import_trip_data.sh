...
for filename in raw_tlc_data/green_tripdata*.csv; do
  ...
  sed $'s/\r$//' $filename | sed '/^$/d' | psql nyc-ecommerce-analysis -c "COPY green_tripdata_staging ${schema} FROM stdin CSV HEADER;"
  echo "`date`: finished raw load for ${filename}"
  psql nyc-ecommerce-analysis -f setup_files/populate_green_trips.sql
  echo "`date`: loaded trips for ${filename}"
done;

for filename in raw_tlc_data/yellow_tripdata*.csv; do
  [[ $filename =~ $year_month_regex ]]
  year=${BASH_REMATCH[1]}
  month=$((10#${BASH_REMATCH[2]}))
  ...
  echo "`date`: beginning load for ${filename}"
  sed $'s/\r$//' $filename | sed '/^$/d' | psql nyc-ecommerce-analysis -c "COPY yellow_tripdata_staging ${schema} FROM stdin CSV HEADER;"
  echo "`date`: finished raw load for ${filename}"
  psql nyc-ecommerce-analysis -f setup_files/populate_yellow_trips.sql
  echo "`date`: loaded trips for ${filename}"
done;

psql nyc-ecommerce-analysis -c "CREATE INDEX ON trips USING BRIN (pickup_datetime) WITH (pages_per_range = 32);"
